...
public class AIService : IAIService
{
    ...
    {
        ChatHistory history = LoadChatHistory();
        ...
        })
        {
            chat.AfterTokenSampling += async (sender, token) =>
            {
                ...
                {
                    await SaveChatHistory(chat.ChatHistory);
                    return;
                }
    ...
    private async Task SaveChatHistory(ChatHistory chatHistory)
    {
        ...
        await _distributedCache.SetStringAsync("chat-history", json);
    }
    private ChatHistory LoadChatHistory()
    {
        var chatHistory = new ChatHistory(_model);
        var json = _distributedCache.GetString("chat-history");
        if (json != null && json != "")
        {
